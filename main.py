...
async def send_telegram(text):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print(f"[TELEGRAM DESATIVADO] {text}")
        return
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as resp:
                if resp.status != 200:
                    print(f"[Telegram erro] {await resp.text()}")
...
def calc_macd(closes):
    ...
    if len(macd_series) >= 9:
        signal_line = calc_ema(macd_series, 9)
    elif macd_series:
...
def analyze_signal(symbol):
    ...
    ema200 = calc_ema(closes, 200) if len(closes) >= 200 else calc_ema(closes, int(len(closes) * 0.75))
    ...
    score_buy = sum([ema_buy * 3, macd_buy * 2, rsi_buy * 2, (price > ema20) * 1])
    score_sell = sum([ema_sell * 3, macd_sell * 2, rsi_sell * 2, (price < ema20) * 1])

    conf = round(max(score_buy, score_sell) / 8 * 100)

    if score_buy >= 7 and score_buy > score_sell:
        return {
            "signal": "BUY", "confidence": conf, "price": price, "atr": atr,
            "tp1": price + atr * 2, "tp2": price + atr * 3.5, "sl": price - atr * 1.5,
        }
    if score_sell >= 7 and score_sell > score_buy:
        return {
            "signal": "SELL", "confidence": conf, "price": price, "atr": atr,
            "tp1": price - atr * 2, "tp2": price - atr * 3.5, "sl": price + atr * 1.5,
        }
    return None
...
def msg_tp1(market_name, signal, tp1):
    ...
    return (f"✅ <b>TP1 ATINGIDO — {market_name}</b>\n"
            ...
            f"💡 Considere mover o stop para o ponto de entrada.")
...
def msg_sl(market_name, signal, sl):
    ...
    return (f"🛑 <b>STOP LOSS ATINGIDO — {market_name}</b>\n"
            ...
            f"📌 Operação encerrada. Aguardando próximo sinal.")

# ── Verificar TP/SL a cada tick ──────────────────────────────
...
# ── Verificar sinal a cada novo candle H1 ───────────────────
    ...
    while True:
        try:
            ...
            async with websockets.connect(uri) as ws:
                # Pedir histórico de 200 candles H1
                ...
                async for raw in ws:
                    ...
                    if msg.get("msg_type") == "candles" and msg.get("candles"):
                        ...
                        print(f"[{symbol}] {len(state[symbol]['candles'])} candles H1 carregados.")
                        # Subscrever ticks e candles ao vivo
                        ...
                        print(f"[{symbol}] Erro WS: {msg['error']['message']}")
...
# ── Iniciar ──────────────────────────────────────────────────
async def main():
    print("🚀 Bot Deriv iniciado — monitorando 10 mercados H1")
    await send_telegram(
