"""
bot.py — Sinais EURUSD para Telegram + monitoramento TP/SL em tempo real
"""
...
def format_tp_hit(tp_num, tp_price, entry, sl, direction, remaining_tps):
    ...
    remaining_str = "".join(f" {l}: `{p:.5f}`\n" for l, p in remaining_tps)

    msg = (
        f"✅ *TP{tp_num} ATINGIDO — {config.SYMBOL_DISPLAY} {tag}*\n"
        f"\n"
        f"🎯 TP{tp_num}: `{tp_price:.5f}`  _(+{pct:.4f}%)_\n"
        f"📈 Entrada foi: `{entry:.5f}`\n"
        f"🛑 SL atual: `{sl:.5f}`\n"
    )
    if remaining_tps:
        msg += f"\n🎯 *Próximos alvos:*\n{remaining_str}"
...
def monitor_signal(sig: dict) -> None:
    ...
    log.info(f"[Monitor] {d} | Entrada={entry:.5f} | SL={sl:.5f}")
    ...
    while remaining:
        ...
        if (price <= sl) if is_buy else (price >= sl):
            log.info(f"[Monitor] SL atingido em {price:.5f}")
            try:
        ...
        if (price >= tp_price) if is_buy else (price <= tp_price):
            ...
            remaining.pop(0)
            log.info(f"[Monitor] {label} atingido em {price:.5f}")
...
def run_job() -> None:
    ...
    if tracker.is_on_cooldown(direction):
        log.info(f"Cooldown ativo — {tracker.remaining_cooldown(direction)}s restantes.")
        return
