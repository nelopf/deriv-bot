"""
bot.py — Envio de sinais ao Telegram + monitoramento TP/SL em tempo real
         Jump 25 Index via API WebSocket da Deriv
"""
...
def format_tp_hit(tp_num: int, tp_price: float, entry: float,
    ...
    for label, price in remaining_tps:
        remaining_str += f" {label}: `{price:.4f}`\n"
...
def monitor_signal(sig: dict) -> None:
    ...
    log.info(f"[Monitor] Iniciado | {d} | Entrada={entry:.4f} | SL={sl:.4f}")
...
def start() -> None:
    ...
    try:
        ...
        send_msg(
            ...
            f"📊 Timeframes: `M15 | M60 | H4`\n"
            f"💪 Confiança mínima: `{config.MIN_CONFIDENCE}%`\n"
            f"\n"
            f"⏳ _Aguardando sinal de alta qualidade..._"
        )
        log.info("Mensagem de início enviada ao Telegram.")
    except Exception as e:
        log.error(f"Erro ao enviar mensagem de início: {e}")
