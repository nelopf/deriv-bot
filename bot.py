...
async def send_signal(signal: dict, symbol: str, symbol_name: str, tick_size: float):
    emoji = "🟢" if signal["type"] == "BUY" else "🔴"
    arrow = "📈" if signal["type"] == "BUY" else "📉"
    tp1, tp2, tp3, sl = calculate_targets(signal["price"], signal["type"], tick_size)

    msg = (
        f"{emoji} *SINAL {signal['type']} — {symbol_name}*\n"
        f"━━━━━━━━━━━━━━━\n"
        f"📊 *Ativo:* {symbol_name}\n"
    ...
    while True:
        try:
            async with websockets.connect(DERIV_WS_URL) as ws:
                ...
                print(f"[BOT] Inscrito em {symbol_name}. Aguardando sinais...")
                    ...
                    if "tick" in data:
                        ...
                        if active_signal is None and cooldown_ok:
                            ...
                            if signal and signal["type"] != last_signal_type:
                                ...
                                tp1, tp2, tp3, sl = calculate_targets(signal["price"], signal["type"], tick_size)
                                await send_signal(signal, symbol, symbol_name, tick_size)
...
async def main():
    print("[BOT] Iniciando bot — Step Index + BTCUSD...")
