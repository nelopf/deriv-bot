"""
analyzer.py — Análise técnica com OHLCV da Binance
              Preço em tempo real via API da Deriv (BTCUSD)
"""
...
# ─── EXCHANGE (OHLCV para indicadores) ────────────────────────────────────────
...
def get_deriv_price() -> float:
    """
    Busca o preço atual do BTCUSD direto na API REST da Deriv.
    Fallback para Binance se a Deriv não responder.
    """
    try:
        url = "https://api.deriv.com/api/v2/ticks/frxBTCUSD"
...
def fetch_ohlcv(timeframe: str, limit: int = config.LOOKBACK_CANDLES) -> pd.DataFrame:
    """Busca velas da Binance e calcula indicadores técnicos."""
