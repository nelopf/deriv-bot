"""
analyzer.py — Coleta OHLCV do Jump 25 Index via WebSocket da Deriv
...
# ─── BUSCAR CANDLES VIA WEBSOCKET DERIV ───────────────────────────────────────

def fetch_candles(granularity_seconds: int, count: int = config.LOOKBACK_CANDLES) -> pd.DataFrame:
    """
    Busca velas OHLCV do Jump 25 via WebSocket da Deriv.
    granularity_seconds: 900=15m, 3600=1h, 14400=4h
    """
    ...
    if result["error"]:
        raise Exception(f"Deriv WebSocket: {result['error']}")
    if not result["data"]:
        raise Exception("Sem dados recebidos da Deriv")
    ...
    df["volume"] = 1  # Deriv não fornece volume real
...
def get_live_price() -> float:
    """Busca o preço atual do Jump 25 via WebSocket da Deriv."""
    ...
    if result["error"]:
        raise Exception(f"Erro ao buscar preço: {result['error']}")
...
def analyze(df: pd.DataFrame) -> dict:
    ...
    if crossed_up:
        ...
        buy_pts += 0.5
        details["MACD"] = ("⚪", f"histograma +{last['macd_hist']:.2f}")
    else:
        sell_pts += 0.5
        details["MACD"] = ("⚪", f"histograma {last['macd_hist']:.2f}")
    ...
    if last["close"] <= last["bb_lower"] and bb_width > 0.005:
        buy_pts += 1
        details["Bollinger"] = ("🟢", "toque banda inferior")
    elif last["close"] >= last["bb_upper"] and bb_width > 0.005:
        sell_pts += 1
        details["Bollinger"] = ("🔴", "toque banda superior")
    else:
        details["Bollinger"] = ("⚪", f"largura {bb_width:.4f}")
...
def get_consensus() -> dict | None:
    results    = {}
    tf_seconds = list(config.TIMEFRAME_SECONDS.items())  # [("M15",900), ...]
    ...
    for tf_name, tf_sec in tf_seconds:
        try:
            ...
            print(f"[ERRO] Timeframe {tf_name}: {e}")
            return None
    ...
    # Preço real em tempo real
    try:
        price = get_live_price()
    except Exception as e:
        print(f"[WARN] Usando preço do candle: {e}")
        base  = results.get("M60") or list(results.values())[0]
        price = base["price"]
