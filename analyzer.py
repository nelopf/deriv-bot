"""
analyzer.py — Coleta OHLCV do EURUSD via Alpha Vantage
"""
...
def fetch_ohlcv(timeframe: str) -> pd.DataFrame:
    """Busca velas OHLCV do EURUSD via Alpha Vantage."""

    if timeframe == "4h":
        # Busca 60min e agrupa em 4h
        interval = "60min"
    else:
        interval = timeframe  # "15min" ou "60min"

    params = {
        "function":    "FX_INTRADAY",
        "from_symbol": config.FROM_CURRENCY,
        "to_symbol":   config.TO_CURRENCY,
        "interval":    interval,
        "outputsize":  "full",
        "apikey":      config.AV_API_KEY,
    }

    resp = requests.get(config.AV_BASE_URL, params=params, timeout=15)
    data = resp.json()

    key = f"Time Series Forex ({interval})"
    if key not in data:
        msg = data.get("Information") or data.get("Note") or str(data)
        raise Exception(f"Alpha Vantage erro [{timeframe}]: {msg}")
...
def get_live_price() -> float:
    """Busca preço atual EURUSD via Alpha Vantage CURRENCY_EXCHANGE_RATE."""
    params = {
...
def get_consensus() -> dict | None:
    ...
    for tf in config.TIMEFRAMES:
        try:
            ...
            time.sleep(12)  # Alpha Vantage free: max 5 req/min
    ...
    try:
        ...
        base  = results.get("60min") or list(results.values())[0]
        price = base["price"]
